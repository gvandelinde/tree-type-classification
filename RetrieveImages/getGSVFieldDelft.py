...
# LANDCOVER_FILENAME = 'data/DelftOverlappedLandCoverRoadsThailandESA-WorldCover-sides20x20.csv'
# TREECOVER_FILENAME = 'data/DelftOverlappedTreeCoverRoadsThailandESA-WorldCover-sides20x20.csv'
ROADPOINTS_FILENAME = 'RoadPointsGeneration/roadPoints/roadPointsDelft2024-03-19_00-31-32.csv'
            ...
            if resJson['status'] ==  'OK':
                ...
                if True:
                    if resJson['pano_id'] not in uniqueImageIDs:
                        ...
                        print("Distance (m) ", distance, "Bearing ", bearing)
                        uniqueImageIDs.append(resJson['pano_id'])
                        lat_lon_str = f"{pt1[1]}_{pt1[0]}".replace('.', '-')
                        print("lat_lon_str", lat_lon_str)
                        meta = f"{i}_"+ resJson['date'] + "_" +lat_lon_str
                        getStreet(lat,lon, myloc, bearing, meta)

                        # bearing = computeBearing(fro, pt2)
                        bearing_opposite = (bearing + 180) % 360
                        distance = computeDistance(fro,pt2)
                        print("Distance (m) ", distance, "Bearing ", bearing_opposite)
                        uniqueImageIDs.append(resJson['pano_id'])
                        lat_lon_str = f"{pt2[1]}_{pt2[0]}".replace('.', '-')
                        meta = f"{i}_"+resJson['date']  + "_" + lat_lon_str
                        getStreet(lat,lon, myloc, bearing_opposite, meta)
        i+=1
